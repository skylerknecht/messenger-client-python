...
try:
    ...
    def decrypt(key: bytes, ciphertext: bytes) -> bytes:
        ...
        # contain the IV
        iv = ciphertext[:16]
        cipher = AES2.new(key, AES2.MODE_CBC, iv)
        msg = unpad(cipher.decrypt(ciphertext[16:]), AES2.block_size)
        return msg
    ...
    def encrypt(key: bytes, plaintext: bytes) -> bytes:
        ...
        iv = Random.new().read(AES2.block_size)
        cipher = AES2.new(key, AES2.MODE_CBC, iv)
        msg = cipher.encrypt(pad(plaintext, AES2.block_size))
        return iv + msg
except:
    def encrypt(key: bytes, plaintext: bytes) -> bytes:
        # Encrypt the plaintext bytes with a provided key.
