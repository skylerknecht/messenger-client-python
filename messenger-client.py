...
async def main():
    ...
    if server.startswith('ws'):
        ...
        client = WSClient(server, encryption_key, user_agent, proxy, remote_port_forwards)
    else:
        server = server.strip('/') + '/socketio/?EIO=4&transport=polling'
        client = HTTPClient(server, encryption_key, user_agent, proxy, remote_port_forwards)
